...
const MaxTokenEntry = ({ maxToken, ifo }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent = t(
    'For the public sale, each eligible participant will be able to commit any amount of BLDT up to the maximum commit limit, which is published along with the IFO voting proposal.',
  )
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('BLDT to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
