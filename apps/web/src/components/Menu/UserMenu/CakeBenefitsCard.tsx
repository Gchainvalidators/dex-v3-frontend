...
const CakeBenefitsCardWrapper = styled(Box)`
  ...
  background: linear-gradient(180deg, #53dee9, #0351EA);
...
const CakeBenefitsCard: React.FC<React.PropsWithChildren<CakeBenefitsCardProps>> = ({ onDismiss }) => {
  ...
  const {
    ...
    <>
      <Text>
        {t(`%lockedCake% BLDT (including rewards) are locked in the BLDT Pool until %lockedEndTime%`, {
  ...
  const {
    ...
    <>
      <Text>
        {t(`iCAKE allows you to participate in the IFO public sales and commit up to %iCake% amount of BLDT.`, {
  ...
  const {
    ...
    <>
      <Text>{t(`bCAKE allows you to boost your yield in BlueLotusDAO Swap Farms by up to 2x.`)}</Text>
  ...
  const {
    ...
    <>
      <Text>
        {t(`vCAKE boosts your voting power to %totalScore% in the BlueLotusDAO Swap voting governance.`, {
        ...
        <>
          ...
          <Message mt="8px" mb="16px" variant="warning">
            <MessageText maxWidth="200px">
              {t(
                'Lock BLDT to enjoy the benefits of farm yield boosting, participating in IFOs, voting power boosts, and so much more!',
              )}{' '}
              <NextLinkFromReactRouter
