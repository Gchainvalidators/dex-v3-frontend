  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="gradientCardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        <Text bold>{t('It’s a BLDT only feature')}</Text>
        ...
        {canSwitch ? (
          <Button
            ...
            isLoading={isLoading}
            onClick={() => (isWrongNetwork ? switchNetworkLocal(ChainId.GSYS) : switchNetworkAsync(ChainId.GSYS))}
          >
            {t('Switch to %chain%', { chain: 'GSYS Smart Chain' })}
          </Button>
        ) : (
