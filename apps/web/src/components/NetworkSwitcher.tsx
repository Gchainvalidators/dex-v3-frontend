  ...
  return (
    <>
      ...
      {chains
        .filter(
          (chain) =>
            Number(chain.id) === ChainId.LINEA_TESTNET || !('testnet' in chain && chain.testnet) || chain.id === chainId,
        )
        .map((chain) => (
          <UserMenuItem
            key={chain.id}
...
const WrongNetworkSelect = ({ switchNetwork, chainId }) => {
  ...
  const localChainId = useLocalNetworkChain() || ChainId.GSYS
  const [, setSessionChainId] = useSessionChainId()

  const localChainName = chains.find((c) => c.id === localChainId)?.name ?? 'GSYS'
