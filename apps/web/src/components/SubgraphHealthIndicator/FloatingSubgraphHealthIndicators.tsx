...
export const V3SubgraphHealthIndicator = subgraphHealthIndicatorFactory({
  getSubgraphName: (chainId) => {
    if (V3_SUBGRAPH_URLS[chainId]?.startsWith('https://thegraph.blue20bridge.io/subgraphs/name/')) {
      return V3_SUBGRAPH_URLS[chainId].replace('https://thegraph.blue20bridge.io/subgraphs/name/', '') || ''
    }
    if (V3_SUBGRAPH_URLS[chainId]?.startsWith('https://thegraph.blue20bridge.io/subgraphs/name/')) {
      return V3_SUBGRAPH_URLS[chainId].replace('https://thegraph.blue20bridge.io/subgraphs/name/', '') || ''
    }
...
export const LotterySubgraphHealthIndicator = subgraphHealthIndicatorFactory({
  getSubgraphName: () => GRAPH_API_LOTTERY.replace('https://thegraph.blue20bridge.io/subgraphs/name/', ''),
})

export const PredictionSubgraphHealthIndicator = subgraphHealthIndicatorFactory({
  getSubgraphName: () => GRAPH_API_PREDICTION_BNB.replace('https://thegraph.blue20bridge.io/subgraphs/name/', ''),
})
