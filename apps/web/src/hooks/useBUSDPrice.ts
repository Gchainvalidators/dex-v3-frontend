...
export function useStablecoinPrice(
  ...
  const isCake = currency && BLDT[chainId] && currency.wrapped.equals(BLDT[chainId])
  ...
  const price = useMemo(() => {
    ...
    //   return new Price(currency, currency, '1', '100')
    // }
...
export const useBNBBusdPrice = (
  ...
  const wgsys: Token = isTestnet ? WBNB[ChainId.BSC_TESTNET] : WGSYS[ChainId.GSYS]
  return usePriceByPairs(USDC[wgsys.chainId], wgsys)
}
