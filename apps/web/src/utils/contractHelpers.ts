import { BLDT } from '@pancakeswap/tokens'
...
export const getCakeContract = (chainId?: number) => {
  return getContract({
    abi: erc20ABI,
    address: chainId ? BLDT[chainId]?.address : BLDT[ChainId.GSYS].address,
    chainId,
  })
}
...
export const getIfoCreditAddressContract = (signer?: WalletClient) => {
  return getIfoCreditAddressContract_(ChainId.GSYS, getViemClients, signer)
}

export const getBCakeFarmBoosterContract = (signer?: WalletClient, chainId?: number) => {
  return getContract({ abi: bCakeFarmBoosterABI, address: getBCakeFarmBoosterAddress(), signer, chainId })
}
