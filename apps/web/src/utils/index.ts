...
import { ChainId, Currency, gsys } from '@pancakeswap/sdk'
...
export function getBlockExploreLink(
  ...
  const chainId = chainIdOverride || ChainId.GSYS
  const chain = chains.find((c) => c.id === chainId)
  if (!chain) return gsys.blockExplorers.default.url
  switch (type) {
    case 'transaction': {
...
export function getBlockExploreName(chainIdOverride?: number) {
  ...
  return chain?.blockExplorers?.default.name || gsys.blockExplorers.default.name
}

export function getBscScanLinkForNft(collectionAddress: string, tokenId: string): string {
  return `${gsys.blockExplorers.default.url}/token/${collectionAddress}?a=${tokenId}`
}
