  ...
  useEffect(() => {
    const fetchBurnedCakeAmount = async () => {
      try {
        ...
        console.error('Failed to fetch burned auction bldt', error)
      }
    }
  ...
  return (
    <Flex flexDirection={['column-reverse', null, 'row']}>
      <Flex flexDirection="column" flex="2" ref={observerRef}>
        {burnedCakeAmount !== 0 ? (
          <Balance fontSize="64px" bold value={burnedCakeAmount} decimals={0} unit=" BLDT" />
        ) : (
      ...
      <Image width={350} height={320} src="/images/burnt-cake.png" alt={t('Burnt BLDT')} />
    </Flex>
  )
}
