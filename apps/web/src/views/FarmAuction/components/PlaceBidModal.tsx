...
import { bscTokens, BLDT } from '@pancakeswap/tokens'
...
const PlaceBidModal: React.FC<React.PropsWithChildren<PlaceBidModalProps>> = ({
  ...
  const { balance: userCake, fetchStatus } = useTokenBalance(bscTokens.bldt.address)
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient BLDT balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% BLDT or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
      <ExistingInfo>
        <Flex justifyContent="space-between">
          ...
          <Text>{t('%num% BLDT', { num: getBalanceNumber(amount) })}</Text>
        </Flex>
        <Flex justifyContent="space-between">
      ...
      <InnerContent>
        ...
        {isFirstBid && (
          <Text pb="8px" small>
            {t('First bid must be %initialBidAmount% BLDT or more.', { initialBidAmount })}
          </Text>
        )}
        <BalanceInput
        ...
        <Text color="textSubtle" small mt="24px">
          {t('If your bid is unsuccessful, you’ll be able to reclaim your BLDT after the auction.')}
        </Text>
      </InnerContent>
    </StyledModal>
  )
