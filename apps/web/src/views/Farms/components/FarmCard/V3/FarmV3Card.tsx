...
export const FarmV3Card: React.FC<React.PropsWithChildren<FarmCardProps>> = ({ farm, removed, account }) => {
  ...
  const earnLabel = t('BLDT + Fees')
  const { lpAddress } = farm
  const isPromotedFarm = farm.token.symbol === 'BLDT'
  ...
  const aprTooltip = useTooltip(
    <>
      <Text>
        {t(
          'Global APR calculated using the total amount of active & staked liquidity with the pool BLDT reward emissions.',
        )}
      </Text>
      <br />
