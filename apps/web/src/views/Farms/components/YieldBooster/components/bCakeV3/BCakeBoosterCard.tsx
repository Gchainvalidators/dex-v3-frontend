...
export const BCakeProxyCakeBalanceCard = () => {
  ...
  return !isLoading && bCakeProxyBalance > 0 ? (
    <Message marginBottom="8px" variant="warning">
      <MessageText>
        {t(
          'There is %amount% BLDT in the proxy booster contract. In order to harvest that amount you should withdraw, deposit or harvest one of the boosted farms.',
...
export const useBCakeTooltipContent = () => {
  ...
  const tooltipContent = (
    <>
      <Box mb="20px">
        {t(
          'Yield Boosters allow you to boost your farming yields by locking BLDT in the fixed-term staking BLDT pool. The more BLDT you lock, and the longer you lock them, the higher the boost you will receive.',
        )}
      </Box>
      {/* <Box>
    ...
    return (
      <Box>
        ...
        <Text color="textSubtle" fontSize={12} mb="16px">
          {t('An active fixed-term BLDT staking position is required for activating farm yield boosters.')}
        </Text>
