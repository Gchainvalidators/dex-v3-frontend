...
const BoosterTooltip = () => {
  ...
  return (
    <>
      {t(
        `Boost multiplier is calculated based on the staking conditions from both Farms and fixed-term BLDT syrup pool and will be automatically updated upon user actions.`,
      )}
      <LinkExternal
...
const useBCakeMessage = (
  ...
  const bCakeMessage = useMemo(() => {
    ...
    if (!locked) return t('Lock BLDT to activate yield booster')
    if (isLockEnd) return t('Renew your BLDT staking to activate yield booster')
