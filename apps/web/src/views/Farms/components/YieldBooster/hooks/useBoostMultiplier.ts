...
async function getPublicMultiplier({ farmBoosterContract }): Promise<number> {
  const [cAResult, caPercisionResult, boostPercisionResult] = await publicClient({ chainId: ChainId.GSYS }).multicall({
    contracts: [
      {
...
async function getUserMultiplier({ farmBoosterContract, account, pid }): Promise<number> {
  const [multiplierResult, boostPrecisionResult] = await publicClient({ chainId: ChainId.GSYS }).multicall({
    contracts: [
      {
        address: farmBoosterContract.address,
        abi: bCakeFarmBoosterABI,
