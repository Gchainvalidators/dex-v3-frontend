...
/**
 * User (Planet Finance) built a contract on top of our original manual BLDT pool,
...
const planetFinanceBurnedTokensWei = 875000000000000000000n
...
const CakeDataRow = () => {
  ...
  const {
    ...
    async () => {
      const [totalSupply, burned, totalLockedAmount] = await publicClient({ chainId: ChainId.GSYS }).multicall({
        contracts: [
          { abi: erc20ABI, address: gsysTokens.bldt.address, functionName: 'totalSupply' },
          {
            abi: erc20ABI,
            address: gsysTokens.bldt.address,
  ...
  return (
    <Grid>
      <Flex flexDirection="column" style={{ gridArea: 'a' }}>
        <Text color="#75808B">{t('Circulating Supply')}</Text>
        {circulatingSupply ? (
          <Balance decimals={0} lineHeight="1.1" fontSize="24px" bold value={circulatingSupply} color="#122549" />
        ) : (
          <Skeleton height={24} width={126} my="4px" />
        )}
      </Flex>
      <StyledColumn noMobileBorder style={{ gridArea: 'b' }}>
        <Text color="#75808B">{t('Total supply')}</Text>
        {cakeSupply ? (
          <Balance decimals={0} lineHeight="1.1" fontSize="24px" bold value={cakeSupply} color="#122549" />
        ) : (
          <>
            <div ref={observerRef} />
            <Skeleton height={24} width={126} my="4px" />
      ...
      <StyledColumn noMobileBorder style={{ gridArea: 'c' }}>
        ...
        <Balance decimals={0} lineHeight="1.1" fontSize="24px" bold value={50000000} color="#122549" />
      </StyledColumn>
      <StyledColumn noDesktopBorder style={{ gridArea: 'd' }}>
        <Text color="#75808B">{t('Market cap')}</Text>
        {mcap?.gt(0) && mcapString ? (
          <Heading scale="lg" color="#122549">
      ...
      <StyledColumn style={{ gridArea: 'e' }}>
        <Text color="#75808B">{t('Burned to date')}</Text>
        {burnedBalance ? (
          <Balance decimals={0} lineHeight="1.1" fontSize="24px" bold value={burnedBalance} color="#122549" />
        ) : (
          <Skeleton height={24} width={126} my="4px" />
        )}
      </StyledColumn>
      <StyledColumn style={{ gridArea: 'f' }}>
        <Text color="#75808B">{t('Current emissions')}</Text>
