...
const PredictionCardData: IconCardData = {
  icon: <PredictionsIcon width="39px" color="#000000" />,
  background: 'linear-gradient(180deg, #FFB341 0%, #FFDC61 100%)',
  // borderColor: '#ffb237',
  rotation: '-2.36deg',
}
...
const bottomLeftImage = {
  ...
  attributes: [
    { src: 'bottom-left', alt: 'BLDT card' },
    { src: 'green', alt: 'Green BLDT card with up arrow' },
    { src: 'red', alt: 'Red Bldt card with down arrow' },
    { src: 'top-right', alt: 'BLDT card' },
  ],
}
      ...
      <TransparentFrame isDark={theme.isDark}>
        <Flex flexDirection="column" alignItems="center" justifyContent="center">
          <ColoredWordHeading
            ...
            color="#000000"
          />
          <Text color="#263A4D">{t('Provably fair, on-chain games.')}</Text>
          <Text mb="40px" color="#263A4D">
            {t('Win big with BlueLotusDAO.')}
          </Text>
          <Flex m="0 auto" flexDirection={['column', null, null, 'row']} maxWidth="600px">
            <Text color="#263A4D" fontSize={30}>{t('Coming soon')}</Text>
            {/* <Flex
              flex="1"
