...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyCake = ifo.currency === bscTokens.bldt
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of BLDT up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max BLDT entry is capped by minimum between your average BLDT balance in the iCAKE, or the pool’s hard cap. To increase the max entry, Stake more BLDT into the iCAKE',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, Max BLDT entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iCAKE.')}{' '}
        </Text>
        <Text display="inline">
          {t('Lock more BLDT for longer durations to increase the maximum number of BLDT you can commit to the sale.')}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, Max BLDT entry is capped by your average BLDT balance in the iCake. To increase the max entry, Stake more BLDT into the iCake',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. BLDT entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {raisingTokenToBurn && <FooterEntry label={t('BLDT to burn:')} value={raisingTokenToBurn} />}
          <FooterEntry
