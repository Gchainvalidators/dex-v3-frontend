...
const CommitTokenSection: React.FC<React.PropsWithChildren<TokenSectionProps & { commitToken: Token }>> = ({
  ...
  if (commitToken.equals(cakeBnbLpToken)) {
    return <TokenSection primaryToken={bscTokens.bldt} secondaryToken={bscTokens.wbnb} {...props} />
  }
...
const IfoCardTokens: React.FC<React.PropsWithChildren<IfoCardTokensProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough BLDT to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your BLDT.',
    ),
    { placement: 'bottom' },
  )
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Activate BlueLotusDAO Swap Profile to take part in next IFO‘s!')
                : t('You need an active BlueLotusDAO Swap Profile to take part in an IFO!')}
            </MessageText>{' '}
          ...
          {ifov31Msg || (
            <>
              <Text textAlign="center" fontSize="14px">
                {t('To participate in the next IFO, lock some BLDT in the fixed-term staking BLDT pool!')}
              </Text>
