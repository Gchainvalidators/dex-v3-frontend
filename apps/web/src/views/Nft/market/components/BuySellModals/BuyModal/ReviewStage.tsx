  ...
  return (
    <>
      <Flex px="24px" pt="24px" flexDirection="column">
        ...
        <BorderedBox>
          ...
          <ButtonMenu
            ...
            variant="subtle"
          >
            <ButtonMenuItem>GSYS</ButtonMenuItem>
            <ButtonMenuItem>WGSYS</ButtonMenuItem>
          </ButtonMenu>
          <Text small color="textSubtle">
            {t('Total payment')}
          </Text>
          ...
          <Text small color="textSubtle">
            {t('%symbol% in wallet', { symbol: paymentCurrency === PaymentCurrency.GSYS ? 'GSYS' : 'WGSYS' })}
          </Text>
          {!account ? (
          ...
          <Message p="8px" variant="danger">
            <Text>
              {t('Not enough %symbol% to purchase this NFT', {
                symbol: paymentCurrency === PaymentCurrency.GSYS ? 'GSYS' : 'WGSYS',
              })}
            </Text>
          </Message>
        )}
        <Flex alignItems="center">
          <Text my="16px" mr="4px">
            {t('Convert between GSYS and WGSYS for free')}:
          </Text>
          <Button
            as={Link}
            p="0px"
            ...
            external
            variant="text"
            href="/swap?inputCurrency=GSYS&outputCurrency=0xAa7aE83eb30DDdd14A017D4222121776317EA8Ba"
          >
            {t('Convert')}
          </Button>
        </Flex>
      ...
      <Flex px="24px" pb="24px" flexDirection="column">
        ...
        <Button as={Link} external style={{ width: '100%' }} href="/swap?outputCurrency=GSYS" variant="secondary">
          {t('Get %symbol1% or %symbol2%', { symbol1: 'GSYS', symbol2: 'WGSYS' })}
        </Button>
      </Flex>
    </>
  )
