...
const SetPriceStage: React.FC<React.PropsWithChildren<SetPriceStageProps>> = ({
  ...
  const { tooltip, tooltipVisible, targetRef } = useTooltip(
    <>
      <Text>
        {t(
          'When selling NFTs from this collection, a portion of the GSYS paid will be diverted before reaching the seller:',
        )}
      </Text>
      {creatorFeeAsNumber > 0 && (
        <Text>{t('%percentage%% royalties to the collection owner', { percentage: creatorFee })}</Text>
      )}
      <Text>{t('%percentage%% trading fee will be used to buy & burn BLDT', { percentage: tradingFee })}</Text>
    </>,
  )
    ...
    <>
      ...
      <GreyedOutContainer>
        ...
        {priceIsOutOfRange && (
          <Text fontSize="12px" color="failure">
            {t('Allowed price range is between %minPrice% and %maxPrice% WGSYS', {
              minPrice: MIN_PRICE,
              maxPrice: MAX_PRICE,
            })}
          </Text>
      ...
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          ...
          <Text small color="textSubtle">
            {t('Sales are in WGSYS. You can swap WGSYS to GSYS 1:1 for free with BlueLotusDAO Swap.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
