      ...
      {!checkedState && (
        <Message variant="warning" mb="16px">
          <MessageText maxWidth="320px">
            {t(
              'Adding more BLDT will renew your lock, setting it to remaining duration. Due to shorter lock period, benefits decrease. To keep similar benefits, extend your lock.',
            )}
          </MessageText>
        </Message>
      )}
...
const AddAmountModal: React.FC<React.PropsWithChildren<AddAmountModalProps>> = ({
  ...
  // if you locked for 1 week, then add bldt without renew the extension, it's possible that remainingDuration + passedDuration less than 1 week.
  ...
  return (
    <RoiCalculatorModalProvider lockedAmount={lockedAmount}>
      <Modal title={t('Add BLDT')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
        <Box mb="16px">
          <BalanceField
