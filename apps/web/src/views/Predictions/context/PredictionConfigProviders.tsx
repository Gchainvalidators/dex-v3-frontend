...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    ...
    if ([PredictionSupportedSymbol.BNB, PredictionSupportedSymbol.BLDT].includes(initToken)) {
      return initToken
    }

    return PredictionSupportedSymbol.BLDT
  })

  useEffect(() => {
    const upperToken = _toUpper(token as string) as PredictionSupportedSymbol

    if ([PredictionSupportedSymbol.BNB, PredictionSupportedSymbol.BLDT].includes(upperToken)) {
