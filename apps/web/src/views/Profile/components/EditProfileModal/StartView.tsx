...
const StartPage: React.FC<React.PropsWithChildren<StartPageProps>> = ({ goToApprove, goToChange, goToRemove }) => {
  ...
  /**
   * Check if the wallet has the required BLDT allowance to change their profile pic or reactivate
      ...
      {profile.isActive ? (
        <>
          <Message variant="warning" my="16px">
            <MessageText>
              {t(
                "Before editing your profile, please make sure you've claimed all the unspent BLDT from previous IFOs!",
              )}
            </MessageText>
          </Message>
          {showCakeRequireFlow ? (
