...
const FixedTermWrapper = styled(Box)<{ expired?: boolean }>`
  ...
  background: ${({ theme, expired }) => (expired ? theme.colors.warning : 'linear-gradient(180deg, #53dee9, #0351EA)')};
        ...
        <Box>
          <Text>
            {isBoostingExpired
              ...
              : t(
                  'Voting power is calculated using the staking amount and remaining staking duration of the fixed-term BLDT staking position at the block.',
                )}
          </Text>
          <Text bold m="10px 0">
            {`${t('BLDT locked:')} ${formatNumber(lockedCakeBalance, 0, 2)}`}
          </Text>
  ...
  return (
    <ModalInner mb="0">
      <Text as="p" mb="24px" fontSize="14px" color="textSubtle">
        {t(
          'Your voting power is determined by the amount of BLDT you held and the remaining duration on the fixed-term staking position (if you have one) at the block detailed below. BLDT held in other places does not contribute to your voting power.',
        )}
      </Text>
