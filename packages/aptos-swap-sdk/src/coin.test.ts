...
describe('Coin', () => {
  ...
  describe('#equals', () => {
    it('fails if address differs', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BLDT').equals(new Coin(ChainId.TESTNET, ADDRESS_TWO, 18, 'BLDT'))
      ).toBe(false)
    })

    it('false if chain id differs', () => {
      expect(
        new Coin(ChainId.MAINNET, ADDRESS_ONE, 18, 'BLDT').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BLDT'))
      ).toBe(false)
    })

    it('true if only decimals differs', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 9, 'BLDT').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BLDT'))
      ).toBe(true)
    })

    it('true if address is the same', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BLDT').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BLDT'))
      ).toBe(true)
    })

    it('true on reference equality', () => {
      const token = new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'BLDT')
    ...
    it('sortBefore', () => {
      ...
      expect(
        new Coin(ChainId.TESTNET, '0x16::pancake::BLDT', 8, '').sortsBefore(
