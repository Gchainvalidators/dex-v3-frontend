...
export async function farmV3FetchFarms({
  ...
  const [poolInfos, cakePrice, v3PoolData] = await Promise.all([
    ...
    (await (await fetch('https://bluelotusdaotemp.bluelotusdao.org/api/farms/price/cake')).json())?.price,
...
export function getFarmsPrices(
  ...
  const commonPriceFarms = farms.map((farm) => {
    ...
    if (
      ...
      farm.token.chainId in BLDT &&
      farm.token.equals(BLDT[farm.token.chainId as keyof typeof BLDT])
    ) {
      tokenPriceBusd = new BN(cakePriceUSD)
    }
    if (
      quoteTokenPriceBusd.isZero() &&
      farm.quoteToken.chainId in BLDT &&
      farm.quoteToken.equals(BLDT[farm.quoteToken.chainId as keyof typeof BLDT])
    ) {
