...
export function createFarmFetcher(provider: ({ chainId }: { chainId: FarmV2SupportedChainId }) => PublicClient) {
  const fetchFarms = async (
    ...
    const masterChefAddress = isTestnet ? masterChefAddresses[chainId] : masterChefAddresses[chainId]
  ...
  return {
    ...
    isTestnet: (chainId: number) => ![ChainId.BSC, ChainId.ETHEREUM, ChainId.GSYS].includes(chainId),
  }
}
