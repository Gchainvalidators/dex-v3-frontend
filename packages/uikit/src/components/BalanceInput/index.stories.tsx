...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"BLDT" | "USD">("BLDT");
  const conversionUnit = editingUnit === "BLDT" ? "USD" : "BLDT";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "BLDT" ? "USD" : "BLDT";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "BLDT") {
      ...
      setValues({
        BLDT: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
